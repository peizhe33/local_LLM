...
def main():
    ...
    # Test queries
    test_queries = [
        "Out of all the trans for all path points of all timing paths, extract the top 10 trans with the highest value",
        "Extract startpoint and endpoint for the path with the worst slack",
        "Show me all violated paths with negative slack",
        "Count how many paths have positive slack vs negative slack"
    ]
    ...
    while True:
        try:
            # Get user query
            user_query = input("Enter your query (or number for test query): ").strip()
            ...
            # Check if user entered a test query number
            if user_query.isdigit() and 1 <= int(user_query) <= len(test_queries):
                user_query = test_queries[int(user_query) - 1]
                print(f"Using test query: {user_query}")
            
            # Process query
            result = query_system.query_database(user_query)
